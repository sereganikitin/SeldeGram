...
export function StoryViewer({ groups, startIdx, onClose, onDeleted }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = (t: number) => {
      const p = Math.min(1, (t - startRef.current) / STORY_DURATION_MS);
      setProgress(p);
      if (p >= 1) {
        setProgress(0);
        next();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
      ...
      >
        {mediaUrl && story.mediaType.startsWith("video/") ? (
          // eslint-disable-next-line jsx-a11y/media-has-caption
          <video src={mediaUrl} autoPlay playsInline className="max-h-full max-w-full object-contain" />
        ) : mediaUrl ? (
